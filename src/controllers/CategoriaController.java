...
public class CategoriaController {
   ...
   public void actualizar(Categoria categoria){
      try{
         ...
         if(categoria.getId() == 0){
            ...
            consulta = conexion.prepareStatement("UPDATE " + this.tabla + " SET nombre =?, salario_base =?, puesto_destino =?, estado =?, fecha_registro = ? WHERE id = ?");
            consulta.setString(1, categoria.getNombre());
            consulta.setDouble(2, categoria.getSalario_base());
            consulta.setString(3, categoria.getPuesto_destino());
            consulta.setInt(4, categoria.getEstado());
            consulta.setString(5, categoria.getFecha_registro());
            consulta.setInt(6, categoria.getId());
         }
   ...
   public Categoria consultar(int id){
      ...
      try{
          conexion = Conexion.Conectar();
         PreparedStatement consulta = conexion.prepareStatement("SELECT id, nombre, salario_base, puesto_destino, estado, fecha_registro FROM " + this.tabla + " WHERE id = ?" );
